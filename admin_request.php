...
foreach ($db->query("SELECT * FROM Request") as $results) {
    ...
    echo "<td>" . formatTime($results["Time_Spent"]) . "</td>";
...
function PrintPDF() {
  ...
  var opt = {
    filename: filename, 
    title: title, 
    margin: [30, 10, 10, 10], // Add left margin of 20pt
  ...
  // Remove the delete button from the cloned table
  var deleteButtons = tableClone.querySelectorAll('.deletebutton');
